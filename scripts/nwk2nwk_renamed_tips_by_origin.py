...
# busco_table_filename = 'tables/busco_grc_classification_odb10_diptera.tsv'
...
which_GRC_dict = defaultdict(lambda: 'Core')
...
with open(busco_table_filename) as f:
    for line in f:
        ...
        if BUSCO_line[0] == 'BUSCO_id':
            continue
        key = BUSCO_line[1].split('_')[0] + BUSCO_line[0]
        # sp_busco e.g. Ling_40951at7147
        which_GRC_dict[key] = BUSCO_line[1].split('_')[2]
        if BUSCO_line[4] == "Cecidomyiidae":
            origin_dict[key] = 'c'
        else:
            origin_dict[key] = 's'

stderr.write("Extracted " + str(len(origin_dict.keys())) + ' busco + species -> origin records\n')
